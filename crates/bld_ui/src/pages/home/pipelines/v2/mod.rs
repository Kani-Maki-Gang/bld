            ...
            <div class="h-full flex flex-col gap-4">
                ...
                <div class="grow flex gap-4">
                    ...
                    <div class="grow">
                        ...
                        <Show
                            when=move || matches!(selected_menu_item.get(), menu::MenuItem::External)
                            fallback=|| view! {}>
                            <PipelineExternalV2 external=move || pipeline().unwrap().external />
                        </Show>
                        <Show
                            when=move || matches!(selected_menu_item.get(), menu::MenuItem::Variables)
                            fallback=|| view! {}>
                            <PipelineVariablesV2
                                title="Variables"
                        ...
                        <Show
                            when=move || matches!(selected_menu_item.get(), menu::MenuItem::Environment)
                            fallback=|| view! {}>
                            <PipelineVariablesV2
                                title="Environment variables"
                                subtitle="The configured environment variables for this pipeline"
                                no_data_text=move || "No environment variables configured".to_string()
                                variables=move || pipeline().unwrap().environment />
                        </Show>
                        <Show
                            when=move || matches!(selected_menu_item.get(), menu::MenuItem::Artifacts)
                            fallback=|| view! {}>
