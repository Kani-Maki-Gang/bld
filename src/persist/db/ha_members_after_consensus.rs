...
pub fn select(conn: &SqliteConnection, sn: &HighAvailSnapshot) -> anyhow::Result<Vec<HighAvailMembersAfterConsensus>> {
...
pub fn select_last_rows(conn: &SqliteConnection, rows: i64) -> anyhow::Result<Vec<HighAvailMembersAfterConsensus>> {
    debug!("loading the last {} rows high availability members after consensus", rows);
...
pub fn insert_many(conn: &SqliteConnection, models: Vec<InsertHighAvailMembersAfterConsensus>) -> anyhow::Result<Vec<HighAvailMembersAfterConsensus>> {
