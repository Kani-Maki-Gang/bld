...
pub fn insert(conn: &SqliteConnection, sm_last_applied_log: i32) -> anyhow::Result<HighAvailStateMachine> {
    debug!("inserting high availability state machine with last applied log: {:?}", sm_last_applied_log);
    conn.transaction(|| {
...
pub fn update(conn: &SqliteConnection, sm_id: i32, sm_last_applied_log: i32) -> anyhow::Result<HighAvailStateMachine> {
